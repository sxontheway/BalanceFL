...
if __name__ == '__main__':
    ...
    parser.add_argument('--imb_ratio', type=float, default=0.01, choices=[0.01, 0.05, 0.1, 1])
    ...
    for k in range(num_client):
        per_client_data_tmp.append([j.clone() for j in per_client_data[k]])
    test_data_tmp = [j.clone() for j in test_data]
        
    for i in range(num_client):
        p = Fed_client(
            network, criterion, config, 
            per_client_data_tmp, per_client_label, training_num_per_cls, 
            test_data_tmp, test_label, state_list, state_dict_list, eval_result_list, idx=i
            ) 
        p.daemon = True
        p.start()
