...
if __name__ == "__main__":
    ...
    # )
    # parser.add_argument(
    #     "--imb_ratio", type=float, default=1, choices=[0.01, 0.05, 0.1, 1]
    # )
    ...
    criterion = init_criterion(config, img_num_per_cls=train_num_per_cls)
    ...
    for client_i in range(config["fl_opt"]["num_clients"]):  # 10 client
    ...
    train_dataset = dataloader.local_client_dataset(
        train_data, train_label, config, aug=False
    )
    train_loader = torch.utils.data.DataLoader(
        train_dataset, batch_size=64, shuffle=True, num_workers=16, pin_memory=True
    )
