...
class DotProduct_Classifier(nn.Module):
    ...
    def update_weight(self):
        ...
        self.weight_scale.data = torch.ones(self.num_classes).to(
            self.fc.weight.data.device
        )
...
class BBN_ResNet_Cifar(nn.Module):
    def __init__(self, block, num_blocks, l2_norm, feat_dim=128):
        ...
        )  # output size (N*channels[0]*32*32)
        self.layer2 = self._make_layer(
            block, channels[1], num_blocks[1], stride=2
        )  # output size (N*channels[1]*16*16)
        self.layer3 = self._make_layer(
            block, channels[2], num_blocks[2] - 1, stride=2
        )  # output size (N*channels[2]*8*8)
        self.cb_block = block(
            self.in_planes, self.in_planes, stride=1
        )  # output size (N*channels[2]*8*8)
        self.rb_block = block(
            self.in_planes, self.in_planes, stride=1
        )  # output size (N*channels[2]*8*8)
...
class FC(nn.Module):
    def __init__(self, planes, data_dim=9, dropout_rate=0.1):
        super(FC, self).__init__()

        self.in_plane = 200 * data_dim
        layers = []
        self.dropout_rate = dropout_rate
        for i in planes:
            layers.append(self._make_layer(i))
        self.layers = nn.Sequential(*layers)
        self.apply(_weights_init)

    def _make_layer(self, plane):
        layers = [
            nn.Linear(self.in_plane, plane),
            nn.LeakyReLU(),
