...
def init_models(config):
    ...
    if config["dataset"]["name"] == "CUB":
        if "ResNet10s" == config["networks"]["feat_model"]["def_file"]:
            ...
            network["feat_model"] = ResNet(
                BasicBlock, [1, 1, 1, 1], l2_norm=l2_norm, channels=[24, 48, 96, 192]
            ).to(
                device
            )  # 3/8 resnet10
            feat_dim = 192
            ...
            l2_norm = config["networks"]["feat_model"]["params"]["l2_norm"]
            network["feat_model"] = ResNet(
                BasicBlock, [1, 1, 1, 1], l2_norm=l2_norm, channels=[32, 64, 128, 256]
            ).to(
                device
            )  # half resnet10
            feat_dim = 256
            ...
            l2_norm = config["networks"]["feat_model"]["params"]["l2_norm"]
            network["feat_model"] = ResNet(
                BasicBlock, [1, 1, 1, 1], l2_norm=l2_norm
            ).to(
                device
            )  # [64,128,256,512]
            ...
            l2_norm = config["networks"]["feat_model"]["params"]["l2_norm"]
            network["feat_model"] = ResNet(
                BasicBlock, [2, 2, 2, 2], l2_norm=l2_norm
            ).to(device)
        elif "ResNet34" == config["networks"]["feat_model"]["def_file"]:
            l2_norm = config["networks"]["feat_model"]["params"]["l2_norm"]
            network["feat_model"] = ResNet(
                BasicBlock, [3, 4, 6, 3], l2_norm=l2_norm
            ).to(device)
        else:
    ...
    if config["dataset"]["name"] in ["CIFAR10", "CIFAR100"]:
        if "ResNet32" in config["networks"]["feat_model"]["def_file"]:
            # feat_dim = 512
            l2_norm = config["networks"]["feat_model"]["params"]["l2_norm"]
            network["feat_model"] = BBN_ResNet_Cifar(
                BasicBlock, [5, 5, 5], l2_norm, feat_dim
            ).to(device)
        elif "ResNet20" in config["networks"]["feat_model"]["def_file"]:
            # feat_dim = 256
            l2_norm = config["networks"]["feat_model"]["params"]["l2_norm"]
            network["feat_model"] = BBN_ResNet_Cifar(
                BasicBlock, [3, 3, 3], l2_norm, feat_dim
            ).to(device)
        elif "ResNet8" in config["networks"]["feat_model"]["def_file"]:
            # feat_dim = 128
            l2_norm = config["networks"]["feat_model"]["params"]["l2_norm"]
            network["feat_model"] = BBN_ResNet_Cifar(
                BasicBlock, [1, 1, 1], l2_norm, feat_dim
            ).to(device)
        else:
            raise NotImplementedError

    if config["dataset"]["name"] == "IMU":
        network["feat_model"] = FC(
            config["networks"]["feat_model"]["params"]["fc_dims"],
            config["networks"]["feat_model"]["data_dim"],
            config["networks"]["feat_model"]["params"]["dropout_rate"],
        ).to(device)
...
def init_per_classifier(config, cls_number):
    ...
    if "DotProductClassifier" in config["networks"]["classifier"]["def_file"]:
        ...
        classifier = DotProduct_Classifier(
            feat_dim=feat_dim, num_classes=cls_number
        ).to(device)
    return classifier


def init_criterion(config, **kw):
